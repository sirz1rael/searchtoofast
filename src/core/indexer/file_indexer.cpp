...
void FileIndexer::request_folder_content(const std::filesystem::path &p) {
    ...
    for (const auto& item : path_content) {
        ...
        if (!is_dir) {
            ...
            // TODO: After limit is reached, migrate content of files to a more permanent storage like a database
            // And clear the in-memory files set to free up RAM for avoid segfaults
            if (this->files.size() > 1500) {
                std::cout << "Reached 1500 files limit, stopping indexing." << std::endl;
                break;
            }
        }
    }
